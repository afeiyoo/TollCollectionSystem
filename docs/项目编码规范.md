...
2. 尝试尽量减少对同一代码的重复计算。 特别是循环的时候，例如：
...
3. 建议在项目里使用范围 for 循环遍历容器，避免使用Qt的 `foreach` 循环（它会对容器进行临时拷贝，带来额外性能开销）。同时，建议循环变量使用 `const` 修饰，以避免意外修改元素。
   
    ```cpp
    for (const QWidget& widget : container) {
        doSomething(widget);
    }
...
1. 类名以大写字母开头，例如：`class MyClass;`

2. 变量名以小写字母开头，例如：`int myVar;`

4. 枚举名称和值以大写字母开头，例如：
   
    ```cpp
    enum VerticesPerEvent { NoVertices = 0, VerticesForSameHeight = 4, VerticesForDifferentHeight = 6 };
    ```
...
确保一个类对所有被覆盖的函数始终使用`override`，以区分没有被覆盖的。
