...
//C2停止类型
#define STOPTYPE_RESTART  0x01
#define STOPTYPE_RESEND   0x02

//RSU指令类型
typedef struct tagRsuCmd{
   BYTE SelRsu;
   BYTE NewRsctl;
   BYTE CmdType;
...
//B0帧RSU状态
typedef struct tag_B0Data{
   BYTE  RSUStatus;                //rsu主状态参数；0x00表示正常，否则表示异常；
   BYTE  PSAMNum;                  //Psam卡个数
   char  hexRSUTerminalId1[13];    //PSAM卡终端机编号，省标PSAM卡号 (12+1)
...
typedef struct tag_B5Data{
   ...
   unsigned short ETCTradNo;                       //联机交易序号 CPU卡
...
//0015卡基本信息文件(43字节)
typedef struct tag_0015FileData{
...
//   BYTE Seats;                            //新格式0x5X时有效
} T0015FileData;

//0019复合文件消费记录(43字节)
typedef struct tag_0019FileData{
...
//C3帧(只对入口有效)
typedef struct tag_C3Data{
  UINT OBUID;
  T0019FileData File0019;     //实际只需要后40字节
  char hexEnTime[15];         //(7B)yyyymmddhhnnss
} TC3Data;

//
typedef struct tag_EF04ProvFee{
   BYTE ProCode; //省级行政区代码
   UINT Fee;     //金额 3字节
}TEF04ProvFee;

//OBU-ESAM EF04
typedef struct tag_OBUEF04Data{
   T0019FileData File0019;    //实际1-39字节
   T0015FileData File0015;    //卡基本信息0015文件数据1-20字节
   ...
   TEF04ProvFee provFee[20];
} TOBUEF04Data;

//单片式 OBU 入出口信息(DF01/EF02)写入内容
typedef struct tag_EnExInfo{
   char RoadNetNo[5];     //收费路网编号[**新增]
   char EnStation_h[5];     //入口站
...
//C6帧(只对出口有效)
typedef struct tag_C6Data{
  UINT OBUID;
  int ConsumeMoney;          //单位:分
  char hexExTime[15];        //(7B)yyyymmddhhnnss
  T0019FileData File0019;    //43字节
  BYTE OBUTradeType;         //交易类型
  char hexOBUDivFactor[17];  //OBU一级分散因子 16+1  B2帧的issuer
  UINT EF04Offset;           //读EF04偏移量，如314字节为0x01 0x3A
  UINT EF04Len;              //EF04读长度，如91字节为0x00 0x5B
  TOBUEF04Data obuEfo4;          //OBU-ESAM EF04 内容
  TEnExInfo enExInfo;        //单片式 OBU 入出口信息(DF01/EF02)写入内容
} TC6Data;

//C8异常交易指令
typedef struct tag_C8Data{
  UINT OBUID;
  UINT UserSerial;  //用户卡交易序号
  BYTE OpMode;      //00H：仅仅获取 TAC；01H：获取 TAC 后并写入 EF04。
  UINT EF04Offset;  //
  UINT EF04Len;     //
  TOBUEF04Data obuEfo4;
} TC8Data;

typedef struct tag_B3Data{
   UINT  OBUID;                            //OBU号
   BYTE  ErrorCode;                        //执行状态代码，此处取值0x00
...
//B4帧,卡和过车信息
typedef struct tag_B4Data{
   UINT  OBUID;               //OBU号
   BYTE  ErrorCode;           //执行状态代码，此处取值0x00
   UINT CardRestMoney;        //卡余额，高位在前，低位在后；
   T0015FileData File0015;    //卡基本信息0015文件数据
   T0019FileData File0019;    //复合消费文件0019文件数据
...
//PSAM卡授权结果返回
typedef struct tag_BBData
{
   BYTE ErrorCode;
   BYTE AuthResult1[4]; //1字节通道号+1字节状态+2字节sw1sw2
   BYTE AuthResult2[4];
} TBBData;

//地感信息
typedef struct tag_D0Data{
  BYTE FrameType;
  UINT OBUID;
  UINT X;
  UINT Y;
...
public:
   ...
   virtual int  SendC3Cmd(TC3Data &data,BYTE rsctl)=0;
   virtual int  SendC6Cmd(TC6Data &data,BYTE rsctl)=0;
   virtual int  SendC8Cmd(TC8Data &data,BYTE rsctl)=0;
   virtual int  SendC9Cmd(TC9Data &data,BYTE rsctl)=0;
   virtual int  SendCACmd(BYTE rsctl)=0;
   virtual int  SendCBCmd(TCBData &data,BYTE rsctl)=0;
...
extern "C" Q_DECL_EXPORT IRsuCtrl32* GetRsuCtrl32();        //根据RSU类型*
