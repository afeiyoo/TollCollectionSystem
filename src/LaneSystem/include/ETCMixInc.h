...
#include "uRsuCtrl32.h"


// ETC全国联网定义
#define EXCEPT_TRADE_LESSMONEY 1 //余额不足
#define EXCEPT_OBU_STATUS 2      // OBU状态错
#define EXCEPT_BLACK_VEH 50      //黑名单车
#define EXCEPT_CARD_BLACK 51     //黑名单卡*
#define EXCEPT_OBU_BLACK 52      // OBU黑名单
...
#define EXCEPT_LAW_BALCK 124      //路政黑名单
...
#define EXCEPT_ERR_NOVEHPLATE 143       //车牌与抓拍车牌不符
#define EXCEPT_DISAGREEAXIS 144     // 车轴型不符
#define EXCEPT_LAW_BULK 145       //大件运输车
...
{
public:
    //获取当前车辆重量  返回0允许继续交易，返回小于0不允许继续交易; =-99 罕见轴型; =-2 超限; =-1无称重;=-3车牌与抓拍不符;=-4大件运输车拦截; nFlag:(1:大件运输车)
    virtual int getCurVehWeight(int &axisType,int &tollWeight,int &limitWeight,QString &axisWeight,bool &isWeightLow, int &nFlag,QString vehPlate="",QString sEnStation="",BYTE VehicleStatus=0x00, int OBUVehClass=1, int VehicleUserType=0)=0;
    ...
    //黑名单、状态名单临时放行(返回-1即关闭天线,0则临时放行),无颜色车牌
    ...
    //天线状态通知(天线连接是否正常，PSAM卡授权是否正常) 0:天线正常,PSAM卡已授权  1:天线正常,PSAM卡未授权 2:天线异常 3:无配置
    virtual void rsuStatus(int status)=0;
    //车牌入口复位允许交易标记 0:允许交易 -1不允许交易
    virtual int CheckEnExFlag(QString noColorPlate) = 0;
    //获取当前模式0:正常过车模式  1：节假日直通模式  2：车队模式
    virtual int getCurMode() = 0;
};
