...
void MIDeskProcess::onShiftIn()
{
    ...
    if (!authOk) {
        m_mainWindow->showFormErrorHint("车道授权失败", {"该车道未在省中心平台注册", "</br>", "请及时联系运维人员处理!"});
        return;
    }
...
void MIDeskProcess::onUpdateCheckingFinished(const QString &url)
{
    if (url == GM_INSTANCE->m_config->m_systemConfig.updateUrl) {
        ...
        if(networkErrorOccured){
            m_mainWindow->showFormErrorHint("检查更新失败", {"车道与服务端网络连接异常，请稍后再试", "</br>", "<strong>请及时联系运维人员处理!</strong>"});
            return;
        }
        // 3. 更新检查完成，开始加载登班参数
    }
