...
public:
    ...
    // 客货流程及情报板显示区域API
    void setVehMode(const QString &vehMode, const QString &color = "#ffffff");
    void setInfoBoard(const QString info, const QString &color = "#ff0000");
    ...
    virtual void setObuHint(const QString &obuHint, const QString &color = "#1ed760") = 0;
...
protected:
    ...
    // 卡内信息区域初始化（由子类各自实现）
    virtual MgsPageArea *initCardInfoArea();
    // 交易提示信息区域初始化（由子类各自实现）
...
private:
    ...
    // 当前车辆卡内信息显示区域
    MgsPageArea *m_cardInfoArea = nullptr;
    // 交易提示区域
    ...
    QGraphicsOpacityEffect *m_weightLowEffect = nullptr;
    QTimer *m_weightLowBlinkTimer = nullptr;
