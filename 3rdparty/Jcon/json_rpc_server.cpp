    ...
    QVariant& return_value)
{
    ...
    std::tie(method_ns, method_name_without_ns) = namespaceAndMethodName(method_name);
    ...
    QVariant& return_value)
{
    ...
    // 第三方库修改 2025-07-26 兼容Qt5
    int return_type = QMetaType::type(return_type_name);
    if (return_type != QMetaType::Void && return_type != QMetaType::UnknownType) {
    ...
    const QString& method_name)
{
    ...
    if (return_value.type() == QVariant::Invalid) {
        ...
        auto msg = QString("method '%1' has unknown return type: %2")
                       .arg(method_name)
    ...
    const QVariant& value)
{
    ...
    if (value.type() == QVariant::Invalid) {
        ...
        auto msg = QString("unknown return type: %1")
                       .arg(value.type());
        logError(msg);
