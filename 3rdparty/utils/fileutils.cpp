...
bool FileUtils::removeRecursively(const FileName &filePath, QString *error)
{
    ...
    if (fileInfo.isDir()) {
        ...
        QStringList fileNames = dir.entryList(QDir::Files | QDir::Hidden | QDir::System | QDir::Dirs
                                              | QDir::NoDotAndDotDot);
...
bool FileUtils::copyRecursively(const FileName &srcFilePath,
                                const FileName &tgtFilePath,
                                QString *error,
                                const std::function<bool(QFileInfo, QFileInfo, QString *)> &copyHelper)
{
...
void FileUtils::makeSureDirExist(const FileName &dirPath)
{
    QString path = canonicalPath(dirPath).toString();
    QDir dir(path);
    if (dir.exists())
        return;
...
FileNameList FileUtils::getFilesWithSuffix(const FileName &dirPath, const QString &suffix)
{
    QString path = canonicalPath(dirPath).toString();
    QDir dir(path);
    ...
    dir.setNameFilters({QString("*.%1").arg(suffix)});
...
void FileUtils::autoDeleteFiles(const QString &path, const QString &fileExtension, qint64 expiredDay)
{
    qint64 var = expiredDay * (-1);
...
bool FileReader::fetch(const QString &fileName, QIODevice::OpenMode mode)
{
    ...
    if (!file.open(QIODevice::ReadOnly | mode)) {
        m_errorString = tr("Cannot open %1 for reading: %2").arg(QDir::toNativeSeparators(fileName), file.errorString());
        return false;
    }
    m_data = file.readAll();
    if (file.error() != QFile::NoError) {
        m_errorString = tr("Cannot read %1: %2").arg(QDir::toNativeSeparators(fileName), file.errorString());
        return false;
    }
    return true;
}
    ...
    if (!ok && !m_hasError) {
        if (!m_file->errorString().isEmpty()) {
            m_errorString = tr("Cannot write file %1: %2")
                                .arg(QDir::toNativeSeparators(m_fileName), m_file->errorString());
        } else {
...
FileSaver::FileSaver(const QString &filename, QIODevice::OpenMode mode)
{
    ...
    if (mode & (QIODevice::ReadOnly | QIODevice::Append)) {
        ...
        // 支持安全写入，使用 SaveFile。写入到临时文件后，在提交保存到目标文件
...
TempFileSaver::TempFileSaver(const QString &templ)
{
    ...
    if (!tempFile->open()) {
        m_errorString = tr("Cannot create temporary file in %1: %2")
                            .arg(QDir::toNativeSeparators(QFileInfo(tempFile->fileTemplate()).absolutePath()),
...
/*
 * FileName 类是一个轻量级的文件名便捷类。在 Windows 系统中，文件名不区分大小写。
*/
FileName::FileName()
    : QString()
{}
