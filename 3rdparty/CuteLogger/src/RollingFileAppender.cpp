...
RollingFileAppender::RollingFileAppender(const QString &baseFileName)
    : FileAppender(baseFileName)
    , m_frequency(DailyRollover)
    , m_logFilesLimit(0)
{
    // 第三方库修改 2025-05-18 修复日志轮转在同一个日志文件上叠加输出问题
...
void RollingFileAppender::setDatePattern(DatePattern datePattern)
{
    ...
    QString fullFileName = m_baseFileName
                           + QDateTime::currentDateTime().toString(datePatternString());
    setFileName(fullFileName);
}
...
void RollingFileAppender::rollOver()
{
    ...
    // 第三方库修改 2025-05-18 修复日志轮转在同一个日志文件上叠加输出问题
    QString targetFileName = m_baseFileName + m_rollOverSuffix;
